...
def query_disease_associations(diseases, drugs, model_id):
    ...
    for i in tqdm(range(len(drugs))):
        ...
        #After
    output_dir = os.path.join(os.path.dirname(__file__), "..", "data", args.dataset)
    os.makedirs(output_dir, exist_ok=True)
    output_file = os.path.join(output_dir, "DirectPred_result.csv")
    pd.DataFrame(result_list, columns=['DrugBank ID', 'OMIM ID']).to_csv(output_file, index=False)
...
if __name__ == '__main__':
    ...
    # Load the OpenAI API key from environment variable (secure)
